...
export default function StatsCards({ stats, loading }: Props) {
  const cards = [
    ...
    {
      label: "Boîtes en Stock",
      value: stats.total_boites.toLocaleString("fr-FR"),
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    },
    {
      ...
      gradient: stats.low_stock_count > 0 ? "from-red-500 to-rose-500" : "from-slate-500 to-slate-600",
      bgGlow: stats.low_stock_count > 0 ? "bg-red-500/10" : "bg-slate-500/10",
    },
  ];
